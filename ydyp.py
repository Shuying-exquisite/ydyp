...
class YP:
        ...
        if check_data['msg'] == 'success':
            ...
            if today_sign_in:
                ...
                if signin_data['msg'] == 'success':
                    ...
                    st.write(signin_data['msg'])
                    self.log_info(signin_data['msg'])
        else:
            st.write(check_data['msg'])
        ...
        try:
            for _ in range(self.click_num):
                ...
                if 'result' in return_data:
                    st.write(f'✅{return_data["result"]}')
                    successful_click += 1

            if successful_click == 0:
                st.write(f'❌未获得 x {self.click_num}')
        except Exception as e:
            st.write(f'错误信息:{e}')
                    ...
                    if task_type == "month":
                        ...
                        for month in tasks:
                            ...
                            if task_status == 'FINISH':
                                st.write(f'-已完成: {task_name}')
                                continue
                            st.write(f'-去完成: {task_name}')
    ...
    def wxsign(self):
        ...
        if return_data['msg'] != 'success':
            return st.write(return_data['msg'])
        if not return_data['result'].get('todaySignIn'):
            return st.write('❌签到失败,可能未绑定公众号')
        return st.write('✅签到成功')
        ...
        try:
            for _ in range(self.click_num):
                ...
                if shake_prize_config:
                    st.write(f"🎉摇一摇获得: {shake_prize_config['name']}")
                    successful_shakes += 1
        except Exception as e:
            st.write(f'错误信息: {e}')
        if successful_shakes == 0:
            st.write(f'❌未摇中 x {self.click_num}')
            ...
            if remain_num > 50 - self.draw:
                for _ in range(self.draw):
                    ...
                    if draw_data.get("code") == 0:
                        ...
                        st.write("✅抽奖成功，获得:" + prize_name)
                    else:
                        st.write("❌抽奖失败")
            else:
                pass
        else:
            st.write(draw_info_data.get('msg'))
    ...
    def fruitLogin(self):
        ...
        if token is not None:
            ...
            if doLoginData.get('result', {}).get('islogin') != 1:
                return st.write('❌果园登录失败')
            # 去做果园任务
            self.fruitTask()
        else:
            st.write("果园专区token刷新失败")
    ...
    def fruitTask(self):
        ...
        if check_sign_data.get('success'):
            ...
            if today_checkin == 1:
                ...
                st.write(f'-领取每日水滴: {given_water}')
                st.write(f'-每日雨滴:{color_data.get("result").get("msg")}')
        else:
            st.write('-果园签到查询失败:', check_sign_data.get('msg', ''))
    ...
    def do_fruit_task(self, task_name, task_id, water_num):
        ...
        if do_task_data.get('success'):
            ...
            if get_water_data.get('success'):
                st.write(f'-已完成任务获得水滴: {water_num}')
            else:
                st.write(f'❌领取失败: {get_water_data.get("msg", "")}')
        else:
            st.write(f'❌参与任务失败: {do_task_data.get("msg", "")}')
    ...
    def tree_info(self):
        ...
        if not treeinfo_data.get('success'):
            ...
            st.write(f'-当前小树等级: {tree_level} 剩余水滴: {collect_water}')
            if tree_level in (2, 4, 6, 8):
                ...
                st.write(f'- {openbox_data.get("result").get("msg")}')
    ...
    def receive(self):
        ...
        st.write(f'-当前云朵数量:{total_amount}云朵')
    ...
    def backup_cloud(self):
        ...
        if state == -1:
            ...
            st.write(f'-获得云朵数量:{cur_data.get("result").get("result")}')

        elif state == 1:
            st.write('-已领取本月连续备份奖励')
        self.sleep()
        ...
        if curMonthBackup:
            st.write(f'- 本月已备份，下月可领取膨胀云朵: {nextMonthTaskRecordCount}')
        else:
            st.write('- 本月还未备份，下月暂无膨胀云朵')

        if preMonthBackup:
            if curMonthBackupTaskAccept:
                st.write('- 上月已备份，膨胀云朵已领取')
            else:
                # 领取
                ...
                if receive_data.get("code") != 0:
                    st.write(f'-领取失败:{receive_data.get("msg")}')
                else:
                    cloudCount = receive_data.get('result', {}).get('cloudCount', '')
                    st.write(f'- 膨胀云朵领取成功: {cloudCount}朵')
        else:
            st.write('-上月未备份，本月无膨胀云朵领取')
        ...
        if pushOn == 1:
            ...
            if firstTaskStatus == 3:
                ...
                reward1_data = self.send_request(reward_url, headers = self.jwtHeaders, data = {"type": 1},
                                                 method = "POST").json()
                st.write(reward1_data.get('result', {}).get('description', ''))
            ...
            if secondTaskStatus == 2:
                ...
                reward2_data = self.send_request(reward_url, headers = self.jwtHeaders, data = {"type": 2},
                                                 method = "POST").json()
                st.write(reward2_data.get('result', {}).get('description', ''))

            st.write(f'- 通知已开启天数: {onDuaration}, 满31天可领取奖励')
        else:
            st.write('- 通知权限未开启')
...
if __name__ == "__main__":
    markdown_text = """
# 移动云盘自动化任务程序
点击 [这里](https://shuyingydypoperationdocument.streamlit.app) 访问操作文档官方网站。
"""
    ...
    cookies = re.split(r'[@\n]', token)
    st.write(f"移动硬盘共获取到{len(cookies)}个账号")
